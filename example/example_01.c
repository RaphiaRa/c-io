...
int main()
{
    ...
    if (IO_ERR_HAS(err = io_UnixAcceptor_bind(&acceptor, "/tmp/test"))) {
        goto on_error;
    }
    io_UnixSocket socket = io_UnixSocket_make(&context);
    if (IO_ERR_HAS(err = io_UnixAcceptor_accept(&acceptor, &socket))) {
        goto on_error;
    }
    char buffer[512];
    size_t size = sizeof(buffer);
    if (IO_ERR_HAS(err = io_UnixSocket_read(&socket, buffer, &size))) {
        goto on_error;
    }
    buffer[size] = '\0';
    printf("Msg: %s\n", buffer);
